...
#    b = 1.5
#    np = Int(round(b*m))
#    X = [ mod.(i*z/M, 1) for i in 0:M-1 ]
#    L = fourier_matrix(X[idcs_s], freqs)
#    idcs, s = BSSsubsampling.bss_plain(L, b, verbose = true)
#    np = length(idcs)
#
#    nps[idx] = np
#    mask = sparse(1:np, idcs, 1, np, n)
#    
#    ghat = lsqr(mask*L, y[idcs_s[idcs]])
#
